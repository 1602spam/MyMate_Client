...
#pragma warning restore CS8620 // null check if문을 사용했으므로/정상적인 값이 전송되는 경우만 고려하므로 무시함
        ...
        private static void EstimateObject(KeyValuePair<byte, object> temp)
        {
            ...
            switch (temp.Key)
            {
                case DataType.TOAST:
                    {
                        ToastProtocol.TOAST? toast;
                        toast = temp.Value as ToastProtocol.TOAST;
                        ToastMessage.ToastMessage.RaiseSystemToast(toast.content);
                    }
                    break;
                ...
                case DataType.MESSAGE:
                    {
                        ...
                        string? user = UserContainer.Instance.Items.Values.FirstOrDefault(MdlUser => MdlUser.Code == message.creater).Username;
                        if (user != null)
                        {
                            ToastMessage.ToastMessage.RaiseMessageToast(user, message.content);
#if DEBUG
